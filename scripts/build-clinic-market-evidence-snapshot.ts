...
const sourceFileRules: Record<string, Omit<SourceFileStatus, "file" | "rowCount" | "sha256">> = {
  ...
  "General Regional/annual_net_sales_by_customer_zip.csv": { qualityStatus: "warning", sensitivity: "confidential", allowedUse: "approved_internal_decision_support_query_boundary_only", browserAiExposure: "none", geographicGrain: "one customer-address ZIP x year" },
  ...
  "Clinic/clinic_market_profile_ownership_demographics.csv": { qualityStatus: "warning", sensitivity: "confidential", allowedUse: "approved_internal_decision_support_pending_identity_rule", browserAiExposure: "none", geographicGrain: "one clinic profile row with reported market context" },
  "Clinic/clinic_level_pre_post_ph_orders_prescriptions_sales.csv": { qualityStatus: "warning", sensitivity: "confidential", allowedUse: "approved_internal_decision_support_query_boundary_only", browserAiExposure: "none", geographicGrain: "one clinic x timeframe activity row" },
...
async function buildSourceStatus(input: { approvedInputs: ApprovedInputFile[]; googleAdsReports: GoogleAdsMatchedLocationsReport[]; builtAt: string }) {
  ...
  return sourceStatusManifestSchema.parse({
    ...
    families: [
      family({ sourceFamily: "general_regional", status: "loaded", evidenceStatus: "Reported", qualityStatus: "warning", geographyStatus: "partial_stable_keys", allowedUse: "approved_internal_decision_support_subject_to_file_boundary", files: approvedFiles.filter((file) => file.file.startsWith("General Regional/")), limitations: ["Some market rows lack exact CBSA codes.", "Confidential regional sales rows cannot cross the browser or AI boundary."] }),
      family({ sourceFamily: "clinic", status: "loaded", evidenceStatus: "Reported", qualityStatus: "warning", geographyStatus: "partial_stable_keys", allowedUse: "approved_internal_decision_support_subject_to_identity_and_query_boundary", files: approvedFiles.filter((file) => file.file.startsWith("Clinic/")), limitations: ["Physical clinic identity, outcome, maturity, and approved peer-cohort rules remain unresolved.", "Confidential clinic profile and activity rows cannot cross the browser or AI boundary."] }),
