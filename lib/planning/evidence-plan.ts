...
function marketingItems(): EvidencePlanItem[] {
  return [
    ...
    item({ id: "media_exposure", label: "Media delivery, cost, and campaign history", role: "analysis_input", requiredFor: "Assess reach and contamination", availability: "partial", sourceIds: ["SRC-018"], reason: "A validated 30-day matched-postal Google Ads export is connected for descriptive click distribution, but campaign taxonomy, location semantics, cost comparability, conversion definitions, and history are not yet governed together.", allowedUse: "Describe observed paid-search response and coverage only; do not infer total demand, incrementality, or authorize spend.", nextAction: "Approve campaign, geography, cost, conversion-goal, attribution, and historical-baseline definitions before evaluating performance or experiments." }),
...
function pricingItems(): EvidencePlanItem[] {
  return [
    ...
    item({ id: "competitive_context", label: "Competitor relevance and availability", role: "validity_check", requiredFor: "Compare like products and meaningful competitors", availability: "partial", sourceIds: ["SRC-025", "SRC-028", "SRC-030"], reason: "A dated ZIP-level competitor-category export is connected for observed offer availability, but competitor relevance, SKU/package comparability, landed-price definitions, sampling coverage, and substitutes remain unresolved.", allowedUse: "Describe monitored competitor availability and coverage only; do not treat the sample as a complete local competitor census or authorize a price action.", nextAction: "Approve the competitor set, representative sampling design, SKU/package matching, landed-price treatment, and freshness rules." }),
