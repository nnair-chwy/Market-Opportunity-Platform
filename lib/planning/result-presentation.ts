...
const MARKETING_MORE_SPEND_ACTION = /\b(?:spend|invest)\w*\s+more\b[^?.]{0,40}\b(?:ads?|paid[ -]?search|media)?|\b(?:receive|allocate)\w*\s+more\b[^?.]{0,30}\b(?:spend|budget)\b/i;
...
export function resolveFinalPresentationMode(plan: EvaluationPlan): FinalPresentationMode {
  ...
  const asksForMaterialAction = plan.perspectiveId === "pricing"
    ...
    : plan.perspectiveId === "marketing"
      ? MARKETING_ACTION.test(question) || MARKETING_MORE_SPEND_ACTION.test(question)
