...
export async function proposeEvaluationPlanWithAi(
  ...
  const basePlan = compileEvaluationPlan(question, response.output_parsed.intent, "ai_proposed", perspectiveId, undefined, activeViewId);
  ...
  return compileEvaluationPlan(question, response.output_parsed.intent, "ai_proposed", perspectiveId, framing, activeViewId);
}
