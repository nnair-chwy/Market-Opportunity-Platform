...
export const discoveryInvestigationIntentSchema = z.object({
  ...
  selectedCbsaCodes: z.array(z.string().regex(/^\d{5}$/)).max(MAX_SELECTED_GEOGRAPHIC_CONTEXTS),
  selectedGeographicContexts: z.array(selectedGeographicContextSchema).max(MAX_SELECTED_GEOGRAPHIC_CONTEXTS),
  marketNames: z.array(z.string().trim().min(1).max(180)).max(MAX_SELECTED_GEOGRAPHIC_CONTEXTS),
