...
import { CONSUMER_INSIGHTS_QUERY_VERSION, consumerInsightsManifestSchema, consumerInsightsQuerySchema, type ConsumerInsightsQuery } from "./contracts.ts";
...
export async function queryConsumerInsights(input: ConsumerInsightsQuery, options: { snapshotDir?: string; databasePath?: string } = {}) {
  ...
  const filters = (alias: string, includeBrand = false) => {
    const clauses = [`${alias}.dma_id = ?`]; values.push("dmaId" in parsed ? parsed.dmaId ?? null : null);
  ...
  try {
    ...
    return { query: parsed.query, rows, snapshotVersion: manifest.snapshot_version, queryVersion: CONSUMER_INSIGHTS_QUERY_VERSION, evidenceBoundary: "Registered aggregate consumer-insights evidence only. No DMA-to-CBSA conversion, scoring, causal interpretation, or arbitrary SQL.", allowedUse: manifest.allowed_use, qualityWarnings: manifest.known_issues, sourceId: manifest.source_id };
  } catch (error) {
    await closeDuckDb(handle);
