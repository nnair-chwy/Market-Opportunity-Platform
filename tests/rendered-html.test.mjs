...
test("server-renders the question-to-action-packet workflow", async () => {
  ...
  assert.match(html, /Show map context and legend/);
  assert.doesNotMatch(html, /Public ACS household counts are market context only/);
  assert.match(html, /View A/);
  assert.match(html, /Explore/);
  assert.match(html, /Compare regions/);
  assert.match(html, /Map layers/);
  assert.match(html, /Add view/);
  assert.match(html, /Analysis view/);
